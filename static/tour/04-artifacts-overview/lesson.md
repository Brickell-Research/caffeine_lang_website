...
**Bonus:** if you chose to install the compiler locally, you can run the `caffeine artifacts` command to get an overview of currently supported artifacts. Here is what it looks like as of `v4.4.0`:
...
DependencyRelations: "Declares soft and hard dependencies between services for dependency mapping."
  ...
  relations: "Map of dependency type to list of service names"
    type: { hard: List(String), soft: List(String) }
    required
```
