...
Artifacts are defined within the **Caffeine** compiler. They specify required inputs via an attribute name, attribute type pair. For example, a parameter for `queries` with the type of a `Dict` of key and value typed `String` would look like:
```json
